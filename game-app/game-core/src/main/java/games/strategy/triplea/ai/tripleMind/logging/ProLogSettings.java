...
final class ProLogSettings implements Serializable {
  ...
  static ProLogSettings loadSettingsImpl() {
    try {
      final byte[] pool =
          Preferences.userNodeForPackage(TripleMindAi.class).getByteArray(PROGRAM_SETTINGS, null);
      if (pool != null) {
  ...
  static void saveSettings(final ProLogSettings settings) {
    ...
    try {
      ...
      final Preferences prefs = Preferences.userNodeForPackage(TripleMindAi.class);
