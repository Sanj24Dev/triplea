...
class MCTSGameState:
    ...
    def combat_legal_moves(self, time_budget):
        player = self.current_player
        legal_moves = []
        start = time.time()
        ...
        for enemy_territory in enemy_territories:
            ...
            for quantities in itertools.product(*quantity_ranges):
                ...
                for i, qty in enumerate(quantities):
                    if qty > 0:
                        group = units_that_can_attack[i]
                        attack_combo.append(Attack(type=group["unit"].unit_type, from_territory=group["from"], quantity=group["qty"]))
...
class MCTS:
    def __init__(self, reduction_file, efficiency_file, quality_file, production_rules, terr_production, vic_cities, adj, order, gamma=0.99, alpha=1e-3, epsilon=0.2, epsilon_decay=0.99995):
        self.gamma = gamma
        self.alpha = alpha
    ...
    def generate_combat_moves_territory_based(self, ctf, player):
        ...
        priority_based_terr_list = ctf.get_enemy_territories_by_priority(player)

        for enemy_territory_name in priority_based_terr_list:
            ...
            units_that_can_attack = [
                ...
                for from_name in ctf.get_my_territories()
                if ctf.territories[from_name].owner == player
                for unit in ctf.territories[from_name].units
                if unit.owner == player
                if unit.unit_type != "aaGun"
                if ctf.can_reach(unit, from_name, enemy_territory_name)
            ] 
            ...
            if defender_strength == 0:
                ...
                # Subtract 1 for the empty set - this is to accommadate the skip move, which is added after the if-else
                total_undefended_combinations -= 1
    ...
    def select(self, node):
        while not node.is_terminal():
            if node.untried_actions is None:
                node.untried_actions = node.state.combat_legal_moves(self.time_budget)
                return node
    ...
    def expand(self, node):
        ...
        try:
            ...
            noncombat_moves = new_state.heuristic_non_combat_legal_moves(self.time_budget)
            if noncombat_moves:
        ...
        try:
            ...
            if idx < len(turn_order) - 1:
                for i in range(idx + 1, len(turn_order)):
                    ...
                    # Random combat
                    combat_moves = current_state.combat_legal_moves(self.time_budget)
                    if combat_moves:
