...
std::vector<UnitInfo> get_reachable_units(
    ...
    const std::unordered_map<std::string, int>& unit_move_ranges
) {
...
int calculate_strength(
    ...
    const std::unordered_map<std::string, double>& unit_attack_values
) {
...
std::vector<Move> combat_legal_moves(
    ...
    for (const auto& [enemy_territory_name, enemy_territory] : territories) {
        ...
        for (const auto& quantities : combinations) {
            ...
            move.strength = calculate_strength(move.moves, unit_attack_values);
...
PYBIND11_MODULE(combat_move_gen_cpp, m) {
