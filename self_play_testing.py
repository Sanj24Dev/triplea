...
os.environ["PLAYER_2"] = "startup.PlayerTypes.PLAYER_TYPE_AI_EASY_LABEL"
os.environ["PLAYER_3"] = "startup.PlayerTypes.PLAYER_TYPE_AI_EASY_LABEL"
os.environ["PLAYER_4"] = "startup.PlayerTypes.PLAYER_TYPE_AI_EASY_LABEL"
...
EFFICIENCY_REC = f"{MODEL_NAME}/test/metrics/mcts_efficiency"
OUTCOME_REC = f"{MODEL_NAME}/test/metrics/game_outcome"
QUALITY_REC = f"{MODEL_NAME}/test/metrics/combat_quality"
ROLLOUT_REC = f"{MODEL_NAME}/test/metrics/rollout_efficiency"

NUM_GAMES = 5


def start_agent(player_name, port):
    log = open(f"{MODEL_NAME}/test/player_logs/debug_{port}.log", "w")
...
PROF_GLOB = f"{main_folder}/test/profiles/mcts_*.prof"
TARGET_FILE = "combat_policy_mcts_agent.py"
CSV_OUTPUT = f"{main_folder}/test/profiling/profile_data.csv"
...
for i in range(1, NUM_GAMES + 1):
    ...
    log = open(f"{MODEL_NAME}/test/player_logs/game.log", "w")
